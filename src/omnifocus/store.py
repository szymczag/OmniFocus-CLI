...
class OFocusStore:
    ...
    def from_env(cls) -> OFocusStore:
        ...
        Optional:
            ``OF_ENCRYPTION_PASSPHRASE`` — if set, files are decrypted.
        ...
        passphrase = os.environ.get("OF_ENCRYPTION_PASSPHRASE") or None
        return cls(client=client, passphrase=passphrase)
        ...
        if is_encrypted(data):
            if self._passphrase is None:
                ...
                raise OFEncryptionError(
                    "Bundle is encrypted but OF_ENCRYPTION_PASSPHRASE is not set"
                )
